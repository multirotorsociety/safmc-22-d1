...
def on_press(key): # HOLDING THE KEY will continue triggering this on_press function!
	...
	try:
		...
		if key.char == 'w': # Ascend
			if not accend == 1:
				accend = 1
				guli.GuliVariable("a").setValue(accend)
				print("Ascend")
		elif key.char == 's': # Descend
			if not accend == -1:
				accend = -1
				guli.GuliVariable("a").setValue(accend)
				print("Descend")
			...
			if not pitch == 1:
				pitch = 1
				guli.GuliVariable("p").setValue(pitch)
				print("back")
		elif key.char == 'i': 
			if not pitch == -1:
				pitch = -1
			...
			if not roll == 1:
				roll = 1
				guli.GuliVariable("r").setValue(roll)
				print("right")
		elif key.char == 'j': 
			if not roll == -1:
				roll = -1
...
def on_release(key):
	global yaw,accend,pitch,roll,em
	try:
	#print('{0} release'.format(key))
		if key.char == 'w' or key.char == 's': # Hover
			accend = 0
			guli.GuliVariable("a").setValue(accend)
			print("height 0")
		if key.char == 'a' or key.char == 'd': # Hover
			yaw = 0
			guli.GuliVariable("y").setValue(yaw)
			print("yaw 0")
		if key.char == 'i' or key.char == 'k': # Hover
			pitch = 0
			guli.GuliVariable("p").setValue(pitch)
			print("pitch 0")
		if key.char == 'j' or key.char == 'l': # Hover
			roll = 0
			guli.GuliVariable("r").setValue(roll)
			print("roll 0")
		if key.char == 'c': 
...
def main():
	...
	print("reading data from keyboard")
	macropadMapping()
